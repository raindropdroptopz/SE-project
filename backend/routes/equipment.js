...
const authenticateToken = (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(401)
      .json({ success: false, message: "Token ไม่ถูกต้อง" });
  }
};
...
router.get("/", async (req, res) => {
  try {
    ...
    const [[{ totalEquip }]] = await db.execute(
      "SELECT COALESCE(SUM(stock), 0) as totalEquip FROM equipment",
    );
    const [[{ totalAvailable }]] = await db.execute(
      "SELECT COALESCE(SUM(available), 0) as totalAvailable FROM equipment",
    );
...
router.post("/borrow", async (req, res) => {
  try {
    const { user_email, equipment_id, quantity, borrow_date, return_date } =
      req.body;
...
router.put("/return/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (borrows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "ไม่พบรายการยืม" });
    }
...
// ===============================================
// POST /api/equipment/buy - ซื้ออุปกรณ์ประเภท 'sell' โดยตรง
// ===============================================
router.post("/buy", authenticateToken, upload.single('payment_slip'), async (req, res) => {
  try {
    ...
    const payment_slip_url = '/photo/slips/' + req.file.filename;
    ...
    if (item.type !== "sell") {
      return res
        .status(400)
        .json({ success: false, message: "อุปกรณ์นี้ไม่ได้อยู่ในรายการขาย" });
    }

    if (item.available < requestedQty) {
      return res.status(400).json({
        success: false,
        message: `สินค้าไม่พอ (เหลือ ${item.available} ชิ้น)`,
      });
    }

    const totalPrice = item.price * requestedQty;
    const today = new Date().toISOString().split("T")[0];

    // บันทึกการซื้อในตาราง equipment_bookings (status = 'purchased', wait for admin verify later if needed)
    const [result] = await db.execute(
      `
            INSERT INTO equipment_bookings (user_id, equipment_id, quantity, borrow_date, return_date, status, total_price, payment_slip)
            VALUES (?, ?, ?, ?, NULL, 'purchased', ?, ?)
        `,
      [userId, equipment_id, requestedQty, today, totalPrice, payment_slip_url],
    );
    ...
    res.status(201).json({
      success: true,
      message: `สั่งซื้อ ${item.name} จำนวน ${requestedQty} ชิ้น และแนบสลิปเรียบร้อยแล้ว`,
